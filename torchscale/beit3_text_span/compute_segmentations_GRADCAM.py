...
def eval_batch(model, image, labels, index, args, classifier):
    ...
    representation, _ = model(image=image.to(args.device), normalize = True  ,only_infer = True)
   
    chosen_class = (representation.detach().cpu().numpy() @ classifier).argmax(axis=1)
   
   
    grayscale_cam = cam(input_tensor=image.to(args.device), targets=[ClassifierOutputTarget(chosen_class[0])])
